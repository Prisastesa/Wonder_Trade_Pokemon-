...
L’objectif de cette analyse est de déterminer si les caractéristiques des Pokémon et des dresseurs influencent la qualité des échanges dans le système Wonder Trade. Nous cherchons à comprendre si les échanges sont purement aléatoires ou s'ils répondent à des logiques comportementales précises.
...
La qualité d’un échange est une notion multidimensionnelle que nous approchons ici sous deux angles complémentaires pour pallier le manque de données systématiques sur la satisfaction des utilisateurs :  
\bigskip
...
\bigskip
  ...
  **Mesure objective** (Score de qualité) : Un score construit sur une échelle de 0 à 6, additionnant des caractéristiques rares ou d'optimisation : présence d’un objet, capacité cachée, egg move, Pokérus, IV parfaits et statut shiny.
...
L'observation des données montre que les Pokémon possédant un taux de capture élevé (espèces communes) sont massivement plus présents dans les échanges. Le flux du Wonder Trade semble donc alimenté par l'abondance des captures faciles plutôt que par la rareté.  

\bigskip

**Puissance des Pokémon par origine géographique**
...
L'analyse de la puissance totale (base_total) des Pokémon selon le pays du dresseur révèle des disparités. Les moyennes et médianes varient selon les zones géographiques, suggérant des comportements ou des niveaux d'avancement différents parmi les communautés de joueurs mondiales.  
...
## Modélisation globale de la satisfaction (Régression logistique)

\bigskip

Pour synthétiser ces analyses, nous avons utilisé une régression logistique afin de prédire la probabilité de satisfaction (liked) en fonction de la rareté, de la puissance, de la facilité de capture et du niveau.  
...
**Analyse de convergence** : Le modèle a rencontré des difficultés de convergence dues à la variable isShiny. En effet, la rareté extrême est un prédicteur si radical de la satisfaction que l'algorithme peine à stabiliser les autres coefficients.  

\bigskip

**Conclusion du modèle** : Les caractéristiques intrinsèques du Pokémon (puissance et rareté) apparaissent comme les facteurs les plus déterminants de l'appréciation d'un échange, tandis que les facteurs liés à l'investissement (niveau) ont un impact plus secondaire.  
...
Cette requête permet de comparer le nombre total de Pokémon existants avec ceux réellement échangés, ainsi que le nombre total d’échanges effectués.
...
Cette requête permet d’identifier la proportion de Pokémon échangés au niveau minimal.
...
(SELECT AVG(p.hp)
FROM pokemon p
WHERE p.pokedex_number IN (SELECT pokemonId FROM trades)) AS hp_echanges_distincts,

(SELECT AVG(hp)
FROM pokemon
WHERE pokedex_number NOT IN (SELECT pokemonId FROM trades)) AS hp_non_echanges;

Cette analyse compare les statistiques de points de vie (HP) entre Pokémon échangés et non échangés.

### Types les moins appréciés
...
Cette requête identifie les types de Pokémon les plus souvent rejetés lors des échanges.

### Nombre de Pokémon shiny

SELECT COUNT(*) AS nb_shiny
FROM trades
WHERE isShiny = 'VRAI';

Cette requête calcule la proportion de Pokémon shiny présents dans les échanges.
...
Cette analyse permet de comparer les HP moyens selon la présence d’un Egg Move.

### Capture rate et échanges
...
Cette requête étudie la relation entre le taux de capture et la fréquence des échanges.

### Genre et qualité des échanges
...
Cette requête calcule un score moyen de qualité des échanges selon le genre du dresseur.

### Anonymat et qualité des échanges
...
Cette requête compare les comportements d’échange entre utilisateurs anonymes et identifiés.
